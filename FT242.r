#
# Perform a OPI::FT procedure at each location in a 24-2 pattern (right eye)
...
if (!file.exists("growthPattern.r"))
    stop("Could not find growthPattern.r needed to generate noise.")
...
#
# RETURNS: a list with
#    np = matrix of number of presentations for each location
...
FT242 <- function(eye = "right", primaryStartValue = 25,
    ...
    startF <- function(guess, rw, cl) {
        ...
        makeStimHelper <- function(x, y) {  # returns a function of (db,n)
            ...
            body(ff) <- substitute(
                {s <- list(x = x, y = y, level = dbTocd(db, 10000 / pi), size = 0.43,
                        color = "white", duration = 200, responseWindow = 1500)
        ...
        return(OPI::FT.start(est = round(guess, 0), makeStim = ms,
                    tt = tt[rw, cl],
                    fpr = fp[rw, cl],
                    fnr = fn[rw, cl]
        ))
    ...
    #####################################################
    stepF <- function(state) { return(OPI::FT.step(state)$state) }
...
### ), nrow = 8, ncol = 9, byrow = TRUE)
###
### print(OPI::FT242(eye="left", primaryStartValue = 25, tt = tt, fpv = 0, fnv = 0))